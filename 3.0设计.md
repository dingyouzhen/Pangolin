...
A->D
```
同理 C 和 D 收到消息后也会做同样的处理，而在 `B->A` 之后， A 不会重新传输给 B，所以就不存在通常意义上的广播风暴。
...
因此需要做节点主动连接数的限制，这样才能做到网络性能和系统稳定性能之间的平衡。
...
## 虚拟机实现
在区块链系统中，虚拟机的实现需要跟区块链有着紧密的关系，这种关系不仅体现在系统的集成性，平台的相关性，同时也体现在与区块链的交互性和数据执行结果的一致性。在虚拟机的实现过程中，需要实现以下几大功能：
...
### 合约交互
我们定义 BUMO 系统为 `BumoA`，定义虚拟机代码为 `ContractA`，`BumoA->ContractA` 定义为调用合约，而`ContractA->BumoA` 定义为合约回调
...
```
//BumoA
void BumoA::CallContract(std::string contract)
{
    CallBuVM(contract);
}

void BumoA::Printf(std::string para)
{
    print(para);
}
...
```
交互过程是，通过 `BumoA::CallContract` 接口把 `ContractA` 的合约传入参数并执行，自动调用内置的虚拟机接口，虚拟机产生回调，即在 `ContractA` 调用 `BumoA` 的函数，可以输出 `hello, world` 结果

### 合约互调
我们定义 BUMO 系统为 `BumoA`，定义虚拟机代码为 `ContractA`，`ContractB`, `BumoA->ContractA` 定义为调用合约，而 `ContractA->ContractB`定义为合约之间的相互调用，如下：
...
void BumoA::Transation(std::string dest_name)
{
    std::string contract = findContrat(dest_name)
...
```
交互过程是，通过 `BumoA::CallContract` 接口把 `ContractA` 的合约传入参数并执行，`ContractA` 发起交易 `Transation` 触发合约 `ContractB` 执行，并产生回调，即在 `ContractB` 调用 `BumoA` 的函数，可以输出 `hello, I am ContractB` 结果

### 内置函数
我们假定合约的入口有三个，`main`, `init`, `query`, 其中 `query` 属于查询功能，合约开发者没有必要在此接口中调用写区块链的功能，只能调用读取得功能。为了防止开发者在开发合约中误使用了写区块链的功能，我们做了如下规定内置函数:

- 所有内置函数拥有固定的只读或者可写权限。
- 合约入口函数定义只读和可写权限
- 合约只读入口函数，只能调用只读的内置函数
- 合约可写入口函数，可以调用只读和可写的内置函数
- 定义合约入口函数权限：`main`、`init` 是可写入口，`query` 是只读入口
...
使用 `callContract` 接口产生交易，也就无需支付上链手续费。调试的过程中，可以设置指定调用接口和参数，执行结果并返回日志，提供了较为便捷的调试方法。`callContract` 中，会启动一个全新的[交易环境](#交易环境)，合约的执行将会在这个独立的环境中运行，且不会干扰当前区块交易、打包等状态。伪代码如下：

```
void callContract(std::string contract)
{
    //创建独立的交易环境
